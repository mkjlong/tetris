...
class Fumen{
	...
	setFumen(fumen, load = false){
		this.fumen = fumen;
		if(this.fumen in fumenData){
			this.element.setAttribute('fumen',this.fumen)
			this.load(fumenData[this.fumen])
		}else{


			this.element.setAttribute('fumen',this.fumen)
			if(decoder.decode(this.fumen).length<4 || load){
				this.load();
			}else{
